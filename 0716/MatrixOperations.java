class MatrixOperations {
     ...
     * 【時間排序註解】若矩陣元素包含時間相關數據（如時間戳），加法操作可用於累計時間間隔，
     * 其線性時間複雜度適合處理按時間序列採集的矩陣數據。
     */
     ...
     * 時間複雜度：O(m × n × p)
     * 【時間排序註解】用於時間序列數據的矩陣乘法（如時序特徵組合），雖時間複雜度較高，
     * 但可通過按時間維度分塊計算優化大規模時間數據處理。
     */
     ...
     * 將行與列互換
     * 時間複雜度：O(m × n)
     * 【時間排序註解】可用於調整時間序列矩陣的維度（如將時間軸從行轉為列），便於按時間順序排序或查詢，
     * 線性時間複雜度使其適合實時時間數據處理。
     */
    ...
    /**
     * 列印矩陣，格式化輸出
     * 【時間排序註解】若矩陣包含時間數據，列印時可按時間軸（行或列）排序後輸出，增強時間序列可讀性。
     */
    ...
    /**
     * 檢查矩陣是否為方陣
     * 【時間排序註解】方陣可用於表示時間點之間的關聯矩陣（如時間段相似度矩陣），便於進行基於時間的對稱性分析。
     */
